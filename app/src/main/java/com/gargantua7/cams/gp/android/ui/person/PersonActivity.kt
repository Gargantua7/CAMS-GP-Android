...
class PersonActivity : ExhibitComposeActivity<Person>(), FAB {
    ...
    override fun fab() {
        ...
        user?.let {
            if (it.username == id) super.fab(icons = Icons.Filled.Edit)
            else super.fab(icons = Icons.Filled.Sms)
        }
    }

    override fun fabOnClick(context: ComposeActivity) {
        CAMSApplication.user.value?.let {
            if (it.username == viewModel.id.value) {
                // TODO
            } else {
                // TODO
            }
